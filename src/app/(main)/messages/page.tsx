...
import { Search, Trash2, Loader2, Send, ArrowLeft, MoreVertical, MessageSquare, Reply, X } from 'lucide-react';
...
// Updated Message Interface to include Reply data
interface Message {
  id: string;
  text: string;
  senderId: string;
...
export default function MessagesPage() {
  ...
  const handleSelectUser = async (user: User) => {
    ...
    setSearchQuery('');
    setMobileView('chat');
    setReplyingTo(null); // Reset reply state on user switch
  ...
  const handleSendMessage = async () => {
    ...
    const currentReply = replyingTo; // Capture current reply state
    ...
    try {
      ...
      if (currentReply) {
        messageData.replyTo = {
          ...
          senderName: currentReply.senderId === currentUser.uid ? 'You' : (selectedUser?.displayName || 'User')
        };
      }
    ...
    <div className="flex h-screen w-full bg-slate-50 dark:bg-slate-950 overflow-hidden font-sans text-slate-900 dark:text-slate-100 transition-colors duration-300">
      ...
      `}>
        {selectedUser ? (
          <>
            {/* Chat Header - Fixed */}
            <header className="flex-none h-[73px] sticky flex items-center justify-between px-6 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-100 dark:border-slate-800 z-30  ">
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-6 bg-slate-50/50 dark:bg-slate-950 custom-scrollbar scroll-smooth" ref={scrollAreaRef}>
              {loadingMessages ? (
                ...
                <div className="flex sticky flex-col justify-end min-h-full pb-2">
                  ...
                  {messages.map((msg, index) => {
                    ...
                    return (
                      <div key={msg.id} className="animate-in slide-in-from-bottom-2 duration-300 fade-in group/message">
                        {showDate && (
                          ...
                          <div className={`flex flex-col max-w-[75%] md:max-w-[65%] min-w-0 ${isMe ? 'items-end' : 'items-start'}`}>
                            <div
                              className={`
                                ...
                                break-words whitespace-pre-wrap w-fit transition-all duration-200
                                ${isMe
                            ...
                            >
                              ...
                              {msg.replyTo && (
                                <div className={`mb-2 p-2 rounded-lg text-xs border-l-4 ${isMe ? 'bg-blue-700/50 border-white/30 text-blue-100' : 'bg-slate-100 dark:bg-slate-800 border-slate-300 dark:border-slate-600 text-slate-500'}`}>
                                  <p className="font-bold mb-0.5">{msg.replyTo.senderName}</p>
                                  <p className="truncate line-clamp-1 opacity-80">{msg.replyTo.text}</p>
                                </div>
                              )}
...
<div className={`absolute top-1/2 -translate-y-1/2 flex items-center gap-1 
    ...
    /* 1. Mobile: Always visible (opacity-100) */
    opacity-100 
    
    /* 2. Desktop (Hover-capable): Hidden by default (opacity-0), visible on hover */
    ...
    ${isMe ? '-left-16 flex-row-reverse' : '-right-6 flex-row'}`
                                ...
                                {isMe && (
                                  <button
                                    ...
                                    className="p-1 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-500 hover:text-red-500 hover:bg-red-50 dark:hover:bg-slate-700 transition-colors shadow-sm"
                                    title="Delete"
                                  >
                                    <Trash2 size={14} />
                                  </button>
            ...
            {/* Input Area - Fixed */}
            <footer className="flex-none bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 sticky bottom-0 z-30">
              {/* Reply Preview Banner */}
              {replyingTo && (
                <div className="px-4 py-2 bg-slate-50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between animate-in slide-in-from-bottom-2 duration-200">
                  <div className="flex flex-col pl-2 border-l-4 border-blue-500 text-sm max-w-[90%]">
                    <span className="font-semibold text-blue-600 dark:text-blue-400">
                      Replying to {replyingTo.senderId === currentUser.uid ? 'Yourself' : (selectedUser?.displayName || 'User')}
                    </span>
                    <span className="truncate text-slate-500 dark:text-slate-400 text-xs">
                      {replyingTo.text}
                    </span>
                  </div>
                  <button onClick={cancelReply} className="p-1 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-full text-slate-500 transition-colors">
                    <X size={16} />
                  </button>
                </div>
              )}
